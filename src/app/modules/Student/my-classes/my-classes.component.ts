...
export class MyClassesComponent {
  ...
  async getStudentClassesFromServer() {
    try {
      this.isLoading = true;
      const getOrderByStudentIdResponse: any = await this.apiService.getOngoingOrderByStudentId(this.studentId);
      console.log("getOrderByStudentIdResponse => ", getOrderByStudentIdResponse);
      if (getOrderByStudentIdResponse?.data) {
        this.classesList = getOrderByStudentIdResponse?.data.map(m => ({
          ...m,
          isChangeRequestPending: m?.['change-teacher-request']?.some(s => s?.request_status === 'pending'),
          isChangeRequestReject: m?.['change-teacher-request']?.some(s => s?.request_status === 'reject'),
          isChangeRequestAccept: m?.['change-teacher-request']?.some(s => s?.request_status === 'accept'),
        }));
